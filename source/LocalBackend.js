...
(function() {	
	function selectFiles(multiple, mimeType) {
	...
	Ambience.LocalBackend.prototype = {
		...
		listAdventures: function() {
			return when(['example:adventure']);
		},
		// Files with text contents (adventures in this case).
		downloadTextFile: function(id) {
			if ( id === 'example:adventure' ) {
				// Note that the library operates on JSON (typically returned from a web service), so we return the JSON here rather than an actual adventure instance.
				var file = {
					id: id,
					contents: Ambience.ExampleAdventure.json
				};
				return when(file);
			} else {
				throw new Error('Attempting to load a non-existing example adventure file.')
			}
		},
		...
		selectImageFile: function() {
			return selectFiles(false, 'image/*');
		},
		selectImageFileLabel: 'Select Image',
		selectSoundFiles: function() {
			return selectFiles(true, 'audio/*');
		},
